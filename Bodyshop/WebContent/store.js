...
function loginEvent(event)
{
	...
	// Open the object
	xhr.open('GET', 'http://34.123.28.32:8080/Bodyshop/LoginServlet?mobileNo='+mobileNo+'&password='+password+'&Ajax=Y', true);
	    	    ...
	    	    if(typeof(Storage) !== 'undefined'){
	    			...
	    			if(JSON.parse(localStorage.getItem('items')) === null){
	    				...
	    				localItems.map(data=>{
	    					...
	    					// Instantiate an xhr object
	    			        const xhr = new XMLHttpRequest();
	    			        xhr.open('GET', 'http://34.123.28.32:8080/Bodyshop/AddToCartServlet?prodName='+data.name+'&image='+data.imageSrc+'&quantity='+data.quantity+'&price='+data.price+'&mobileNo='+mobileNo, true);
	    			    	// What to do when response is ready
	    			        xhr.onload = function () {
	    			            if(this.status === 200){
	    			            	// window.location.href='http://34.123.28.32:8080/Bodyshop/productDelivery.jsp';
...
const xhr = new XMLHttpRequest();
// Open the object
xhr.open('GET', 'http://34.123.28.32:8080/Bodyshop/CartFetchServlet', true);
// What to do when response is ready
xhr.onload = function () {
    if(this.status === 200){
...
function orderUpdate(event)
{
 ...
 // Instantiate an xhr object
    const xhr = new XMLHttpRequest();
    xhr.open('GET', 'http://34.123.28.32:8080/Bodyshop/updateOrderServlet?orderAddresss='+orderAddresss+'&orderDeliveryTime='+orderDeliveryTime+'&orderDate='+orderDate+'&OrderStatus='+OrderStatus+'&orderId='+orderId, true);
	// What to do when response is ready
    xhr.onload = function () {
        if(this.status === 200){
        	window.location.href='http://34.123.28.32:8080/Bodyshop/productDelivery.jsp';
...
function searchOrderId(event){
	...
	var searchtext=document.getElementsByName('search1')[0].value
        	...
        	window.location.href='http://34.123.28.32:8080/Bodyshop/productDelivery.jsp?search1='+searchtext;
...
function searchProduct(event){
	...
	var searchtext1=document.getElementsByName('search')[0].value
        	...
        	window.location.href='http://34.123.28.32:8080/Bodyshop/welcome.jsp?search='+searchtext1;
...
function addNewAddress(event){
 ...
 // Instantiate an xhr object
    const xhr = new XMLHttpRequest();
    xhr.open('GET', 'http://34.123.28.32:8080/Bodyshop/AddressFetchServlet?addAddress=newAddress', true);
	// What to do when response is ready
    xhr.onload = function () {
        if(this.status === 200){
        	window.location.reload();  
...
function changeAddress(event){
 ...
 // Instantiate an xhr object
    const xhr = new XMLHttpRequest();
    xhr.open('GET', 'http://34.123.28.32:8080/Bodyshop/AddressFetchServlet?changeAddressId='+changeAddressId, true);
	// What to do when response is ready
    xhr.onload = function () {
        if(this.status === 200){
        	window.location.reload();  
...
console.log("mobile No "+mobileNo)
// Open the object
xhr.open('GET', 'http://34.123.28.32:8080/Bodyshop/AddressFetchServlet?mobileNo='+mobileNo, true);
...
{
const xhr = new XMLHttpRequest();
xhr.open('GET', 'http://34.123.28.32:8080/Bodyshop/RemoveCartServlet?productName='+productName+'&mobileNo='+mobileNo, true);
// What to do when response is ready
xhr.onload = function () {
    if(this.status === 200){
    	window.location.reload();  
...
function addItemToCart(title, price, imageSrc) {
    ...
    if(mobileNo!=null&&mobileNo!=="NotFound")
    {
    	...
    	// Instantiate an xhr object
        const xhr = new XMLHttpRequest();
        xhr.open('GET', 'http://34.123.28.32:8080/Bodyshop/AddToCartServlet?prodName='+title+'&image='+imageSrc+'&quantity=1&price='+price, true);
    	// What to do when response is ready
        xhr.onload = function () {
            if(this.status === 200){
            	// window.location.href='http://34.123.28.32:8080/Bodyshop/productDelivery.jsp';
